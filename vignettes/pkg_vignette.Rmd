...
```
The plot:
```{r the plot, fig.show='hide', eval=FALSE}
library(gridExtra)
library(ggpubr)
...
ggsave(plot_f, filename = paste0(vig,"/wtrep1vswtrep2.png"), device = "png")
```
finally plotting:
```{r the final plot}
knitr::include_graphics(paste0(vig,"/wtrep1vswtrep2.png"))
```
